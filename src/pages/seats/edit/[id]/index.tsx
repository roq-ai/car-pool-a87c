...
import { getSeatById, updateSeatById } from 'apiSdk/seats';
...
function SeatEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<SeatInterface>(
    () => (id ? `/seats/${id}` : null),
    () => getSeatById(id),
  );
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: SeatInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateSeatById(id, values);
      mutate(updated);
      resetForm();
      router.push('/seats');
    } catch (error) {
  ...
  const formik = useFormik<SeatInterface>({
    initialValues: data,
