...
import { getSeats, deleteSeatById } from 'apiSdk/seats';
import { SeatInterface } from 'interfaces/seat';

type ColumnType = ColumnDef<SeatInterface, unknown>;
...
export function SeatListPage(props: SeatListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<SeatInterface>>(
    () => `/seats?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: SeatInterface) => {
    if (hasAccess('seat', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/seats/view/${row.id}`);
    }
  };
...

    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/seats/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
          ...
          {hasAccess('seat', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/seats/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('seat', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="seat" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Seats
        </Text>
        {hasAccess('seat', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/seats/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
